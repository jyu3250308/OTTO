...
> 🆕 **v2 UPDATE (2026-07-23)**: 시뮬레이션이었던 아트 생성이 **진짜 렌더링 엔진**으로 전면 재작성됐습니다!
> 이제 실제 음표 데이터(월광 소나타·바흐·짐노페디·아리랑)가 색·곡선·별자리로 변환된 **실물 PNG 작품**이 `artworks/` 폴더에 저장됩니다.
> `pip install Pillow` 후 실행 — 매번 다른 색감의 세상에 하나뿐인 작품이 나와요 🎨

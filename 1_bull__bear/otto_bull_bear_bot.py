...
import random
import re
import traceback # Added for consistent traceback printing
...
# or passed as arguments to a class instance. For this simulation, globals suffice.
...
def _mock_send_email(recipient_email: str, subject: str, body: str) -> bool:
    ...
    if random.random() < 0.1:
        print(f"[ERROR] '{recipient_email}'에게 이메일 발송 실패 (모의 네트워크 오류 발생 시뮬레이션).")
        return False
    
    print(f"[DEBUG] --- 이메일 내용 (수신자: {recipient_email}) ---")
    # For actual email content, it might be too verbose to print in DEBUG.
    # Printing a snippet or just a confirmation for debugging purposes is generally better.
    # For this simulation, printing the full body for verification is acceptable.
    print(body)
    print("-----------------------------------")
...
def _add_mock_subscriber(email: str) -> None:
    ...
    if email not in subscribers:
        ...
        current_profit += 1.0 # 구독료 $1 시뮬레이션 (LLM API 비용 충당 등)
...
def _get_dashboard_status() -> str:
    ...
    status = (
        ...
        f"활성 구독자 수: {active_subscribers}명 (총 구독자: {len(subscribers)}명)\\n"
        f"다음 뉴스레터 발송 예정 시간: 매일 오전 {NEWSLETTER_SEND_HOUR}시 (현지 시간)\\n"
        f"마지막 뉴스레터 발송일: {last_sent_date.strftime('%Y-%m-%d') if last_sent_date else '없음'}\\n"
        f"------------------------------"
    )
    return status
...
def _send_daily_newsletter() -> None:
    ...
    if last_sent_date == current_date:
        print(f"[INFO] 뉴스레터는 이미 오늘({current_date.strftime('%Y-%m-%d')}) 발송되었습니다. 재발송을 건너뜜니다.")
        return

    print(f"[INFO] ===== {current_date.strftime('%Y-%m-%d')} 일일 뉴스레터 발송 절차 시작 =====")

    try:
        # 1. 금융 뉴스 및 시장 데이터 수집
        print("[INFO] 1/4단계: 모의 금융 뉴스 데이터 수집 중...")
        news_data = _get_mock_financial_news()
        print("[INFO] 1/4단계: 뉴스 데이터 수집 완료.")

        # 2. LLM (모의)을 이용한 분석 및 요약
        print("[INFO] 2/4단계: 모의 LLM을 이용한 시장 분석 및 3줄 요약 중...")
        ...
        if not subscribers:
            print("[WARN] 현재 활성 구독자가 없습니다. 뉴스레터가 발송되지 않습니다.")
            last_sent_date = current_date # Even if no subscribers, mark as sent for today
            print(f"[INFO] ===== 일일 뉴스레터 발송 절차 완료 (구독자 없음) =====")
            return

        active_subscribers_list = [email for email, data in subscribers.items() if data['is_active']]
        if not active_subscribers_list:
            print("[WARN] 활성 상태인 구독자가 없습니다. 뉴스레터가 발송되지 않습니다.")
            last_sent_date = current_date # Mark as sent for today
            print(f"[INFO] ===== 일일 뉴스레터 발송 절차 완료 (활성 구독자 없음) =====")
            return

        for email in active_subscribers_list:
            try:
                if _mock_send_email(email, newsletter_subject, newsletter_body):
                    sent_count += 1
                else:
                    failed_count += 1
                    # Specific error already printed inside _mock_send_email
            except Exception as send_err:
                failed_count += 1
                print(f"[CRITICAL ERROR] '{email}'에게 이메일 발송 중 예외 발생: {send_err}")
                traceback.print_exc() # Print full stack trace for critical errors
        
        last_sent_date = current_date # Mark today's newsletter as sent
        print(f"[INFO] 4/4단계: 일일 뉴스레터 발송 완료. 총 {len(active_subscribers_list)}명 중 {sent_count}명에게 성공, {failed_count}명에게 실패.")
        print(f"[INFO] ===== 일일 뉴스레터 발송 절차 종료 =====")

    except Exception as e:
        print(f"[CRITICAL ERROR] 일일 뉴스레터 발송 중 치명적인 오류 발생: {e}")
        traceback.print_exc()
        print(f"[CRITICAL ERROR] ===== 일일 뉴스레터 발송 절차 비정상 종료 =====")
...
def run_otto_bot() -> None:
    ...
    try:
        ...
        print(f"[ERROR] 초기 모의 구독자 생성 중 오류 발생: {e}")
        traceback.print_exc()

    # 봇 시작 대시보드 출력
    print("\\n" + _get_dashboard_status())
    print(f"[INFO] 봇이 백그라운드에서 실행됩니다. 매일 오전 {NEWSLETTER_SEND_HOUR}시에 뉴스레터 발송을 확인합니다.")
    ...
    try:
        while True:
            ...
            if now.hour == NEWSLETTER_SEND_HOUR and (last_sent_date is None or last_sent_date < now.date()):
                print(f"\\n[INFO] 현재 시각 {now.strftime('%H:%M:%S')}. 오전 {NEWSLETTER_SEND_HOUR}시, 일일 뉴스레터 발송 트리거 조건 충족!")
                _send_daily_newsletter()
                print("\\n" + _get_dashboard_status()) # 발송 후 업데이트된 대시보드 출력

                # 목표 수익 달성 확인
                if current_profit >= TARGET_PROFIT:
                    print(f"\\n!!!! 축하합니다! 목표 순수익 ${TARGET_PROFIT:.2f}를 달성했습니다! !!!!")
                    print("[INFO] 봇이 목표를 달성했으므로, 더 이상의 작동을 중지합니다.")
                    break # 목표 달성 시 봇 작동 중지 (또는 계속 운영하여 추가 수익 창출 가능, 필요에 따라 조정)
                else:
                    print(f"[INFO] 현재 순수익 ${current_profit:.2f}. 목표 ${TARGET_PROFIT:.2f} 미달성. 계속 운영합니다.")
            
            # 다음 확인까지 대기
            # 데모 목적상 5분 대기. 실제 운영 시 대기 시간을 60분 (60*60 초) 등으로 조정할 수 있습니다.
            print(f"[INFO] 현재 시각: {now.strftime('%Y-%m-%d %H:%M:%S')}. 다음 뉴스레터 발송 확인까지 대기 중 (5분 간격).")
            time.sleep(60 * 5) # 5분 대기

    except KeyboardInterrupt:
        print("\\n[INFO] 사용자 요청(Ctrl+C)에 의해 봇이 안전하게 중지됩니다.")
    except Exception as e:
        print(f"[CRITICAL ERROR] 봇 메인 루프에서 예상치 못한 치명적인 오류 발생: {e}")
        traceback.print_exc()
    finally:
...
if __name__ == "__main__":
    ...
    # (실제 환경에서는 영구 저장소에서 마지막 발송일을 로드하여 봇이 다시 시작해도 연속성을 유지합니다.)
    # 예시: last_sent_date = datetime.date.today() - datetime.timedelta(days=1)
    run_otto_bot()

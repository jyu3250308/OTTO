...
def _fetch_url_content(url: str) -> str:
    """URL에서 콘텐츠를 가져옵니다."""
    _log_message(f"🌐 URL '{url}'에서 콘텐츠 요청 시도...")
    try:
        response = requests.get(url, timeout=REQUEST_TIMEOUT)
        response.raise_for_status() # HTTP 오류 발생 시 예외 발생 (4xx, 5xx)
        _log_message(f"✅ URL '{url}'에서 콘텐츠를 성공적으로 가져왔습니다.")
        return response.text
    except requests.exceptions.Timeout:
        _log_message(f"❌ 오류: URL '{url}' 요청 시간 초과.")
    except requests.exceptions.RequestException as e:
        _log_message(f"❌ 오류: URL '{url}'에서 콘텐츠를 가져오는 중 문제가 발생했습니다: {e}.")
    return ""

def _fetch_file_content(file_path: str) -> str:
    """로컬 파일에서 콘텐츠를 읽어옵니다."""
    _log_message(f"📄 로컬 파일 '{file_path}'에서 콘텐츠 읽기 시도...")
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            _log_message(f"✅ 로컬 파일 '{file_path}'에서 콘텐츠를 성공적으로 읽었습니다.")
            return f.read()
    except FileNotFoundError:
        _log_message(f"❌ 오류: 파일 '{file_path}'을(를) 찾을 수 없습니다.")
    except IOError as e:
        _log_message(f"❌ 오류: 파일 '{file_path}'을(를) 읽는 중 문제가 발생했습니다: {e}.")
    return ""
...
def fetch_content_for_keywords(keywords: list[str], source_url: str = None, source_file: str = None) -> dict:
    ...
    if source_url: text_content = _fetch_url_content(source_url); source_type = "URL"
    elif source_file: text_content = _fetch_file_content(source_file); source_type = "File"
    ...
    content_counts = {keyword.lower(): text_content.lower().count(keyword.lower()) for keyword in keywords}
...
def load_history() -> dict[str, list[dict]]:
    ...
    history = {}
    if not os.path.exists(HISTORY_FILE): _log_message(f"ℹ️ 이력 파일 '{HISTORY_FILE}'이(가) 존재하지 않습니다. 새로 생성합니다."); return history
    ...
    try:
        with open(HISTORY_FILE, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                if all(k in row for k in ['keyword', 'timestamp', 'count']):
                    history.setdefault(row['keyword'].lower(), []).append({'timestamp': row['timestamp'], 'count': int(row['count'])})
                else:
        ...
        with open(HISTORY_FILE, 'w', newline='', encoding='utf-8') as f:
            ...
            for keyword, data_list in history.items():
                for data_entry in data_list:
                    if all(k in data_entry for k in fieldnames[:-1]): # timestamp, count
...
def analyze_niche_pulse(current_counts: dict[str, int], history: dict[str, list[dict]]) -> list[dict]:
    ...
    for keyword, current_count in current_counts.items():
        ...
        _log_message(f"  -> '{keyword}': 현재 {current_count}회 (이전 평균 {avg_historical_count:.2f}회, 상태: {status})")
        analysis_results.append({
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'keyword': keyword,
            'current_count': current_count, 'historical_average': round(avg_historical_count, 2), 'status': status
        })
...
def generate_report(results: list[dict]):
    ...
    if not results: _log_message("ℹ️ 보고서로 작성할 분석 결과가 없습니다."); return
...
def main():
    ...
    parser.add_argument('-k', '--keywords', type=str, required=True, help="분석할 키워드를 쉼표로 구분하여 입력하세요 (예: 'AI art,crypto,blockchain').")
    parser.add_argument('-u', '--url', type=str, default=None, help="콘텐츠를 가져올 웹 페이지 URL.")
    parser.add_argument('-f', '--file', type=str, default=None, help="콘텐츠를 가져올 로컬 파일 경로.")
    
    args = parser.parse_args()

    keywords_list = [k.strip() for k in args.keywords.split(',') if k.strip()]
    if not keywords_list: _log_message("❌ 오류: 분석할 키워드가 제공되지 않았습니다. -k 또는 --keywords 옵션을 사용하세요."); sys.exit(1)
    ...
    if not current_keyword_counts or all(v == 0 for v in current_keyword_counts.values()):
        _log_message("❌ 오류: 키워드 빈도를 계산할 콘텐츠를 가져오지 못했거나 유효한 키워드가 발견되지 않았습니다. 프로그램을 종료합니다.")
        sys.exit(1)

...
def _get_content(source_path: str) -> str | None:
    ...
    if source_path.startswith(("http://", "https://")):
        try:
            ...
            response = requests.get(source_path, timeout=15) # 타임아웃 15초
...
def _extract_and_check_links(html_content: str) -> list[str]:
    ...
    for i, link in enumerate(sorted(list(external_links))): # 일관된 출력을 위해 정렬
...
def main():
    ...
    if args.url or args.file:
        ...
        print(f"[정보] 지정된 소스({source_path})로 분석 시작...")

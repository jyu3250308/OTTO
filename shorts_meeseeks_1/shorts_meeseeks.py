...
def fetch_top_memes(limit=10, attempts=3):
    """🌐 오늘의 인기 밈을 실시간 수집합니다 (meme-api.com — 무료·키 불필요, 레딧 인기글 기반).
    외부 서비스가 순간적으로 느릴 수 있어 3회까지 재시도합니다."""
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            last_err = err
            if attempt < attempts:
                print(f"[Meeseeks] 수집 지연({err}) → 재시도 {attempt}/{attempts - 1}...")
                time.sleep(3)
    print(f"[Meeseeks] 실시간 수집 실패({last_err}) → 데모 데이터로 시연합니다. (잠시 후 다시 실행해 보세요!)")
